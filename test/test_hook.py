...
def test_foreach(opt, size: int = 128, depth: int = 1, iterations: int = 32, outer_iterations: int = 1):
    set_torch()
    ...
    for use_hook in [False, True]:
        ...
        for i in range(outer_iterations):
            ...
            if use_hook:
                ...
                o = opt(model.parameters(), lr=1e-3, weight_decay=1e-4, warmup_steps=16)
    ...
    for i, (l0, l1) in enumerate(zip(*losses)):
        ...
        assert torch.allclose(l0.float(), l1.float(), rtol=0.05)
