...
factor after whitening the other factor by its current eigenvalue EMA.
"""
...
def _kl_state(
    ...
    """Extend SOAP's matrix state with one KL eigenvalue EMA per factor."""
    ...
    if "GG_l" in state:
        state["eigenvalues_l"] = torch.ones((rows,), dtype=ref.dtype, device=ref.device) * factor
    if "GG_r" in state:
        state["eigenvalues_r"] = torch.ones((columns,), dtype=ref.dtype, device=ref.device) * factor
    return state
...
def _factor_inverse(eigenvalues: Tensor, eps: Tensor) -> Tensor:
    """Apply a KL eigenvalue inverse for an allocated legacy factor."""

    return eigenvalues.clamp_min(eps).rsqrt()


def _heavy_factor_inverse(eigenvalues: Tensor, eps: Tensor) -> Tensor:
    """Apply the Moore-Penrose KL eigenvalue inverse."""

    return torch.where(eigenvalues > eps, eigenvalues.rsqrt(), torch.zeros_like(eigenvalues))
...
def _apply_kl_preconditioner(
    ...
    inverse_left = _factor_inverse(eigenvalues_left, tempo.hyper.eps) if eigenvalues_left is not None else None
    inverse_right = _factor_inverse(eigenvalues_right, tempo.hyper.eps) if eigenvalues_right is not None else None
    ...
    left_outer = (
        (left_work @ left_work.mT) / columns
        if left_work is not None
        else None
    )
    right_outer = (
        (right_work.mT @ right_work) / rows
        if right_work is not None
        else None
    )
    left_estimate = (
        (projected if inverse_right is None else projected * inverse_right.unsqueeze(-2)).square().sum(dim=-1) / columns
        if gg_left is not None
        else None
    )
    right_estimate = (
        (projected if inverse_left is None else projected * inverse_left.unsqueeze(-1)).square().sum(dim=-2) / rows
        if gg_right is not None
        else None
    )

    leaf_beta = beta.expand_as(tempo.age)
    next_gg_left = _update_gram(gg_left, left_outer, leaf_beta) if gg_left is not None else None
    next_gg_right = _update_gram(gg_right, right_outer, leaf_beta) if gg_right is not None else None
    ...
    next_eigenvalues_left = (
        eigenvalues_left * left_beta + left_estimate * (1 - left_beta) if eigenvalues_left is not None else None
    )
    next_eigenvalues_right = (
        eigenvalues_right * right_beta + right_estimate * (1 - right_beta) if eigenvalues_right is not None else None
    )
...
def _apply_heavy_kl_preconditioner(
    ...
    eigenvalues_right = _wide(state["eigenvalues_r"]) if gg_right is not None else None
    inverse_left = (
        _heavy_factor_inverse(eigenvalues_left, tempo.hyper.eps) if eigenvalues_left is not None else None
    )
    inverse_right = (
        _heavy_factor_inverse(eigenvalues_right, tempo.hyper.eps) if eigenvalues_right is not None else None
    )
    projected = _project(update, left, right, back=False)
    left_work = None
    if gg_left is not None:
    ...
    rows, columns = update.shape[-2:]
    left_outer = (left_work @ left_work.mT) / columns if left_work is not None else None
    right_outer = (right_work.mT @ right_work) / rows if right_work is not None else None
    left_estimate = (
        (projected if inverse_right is None else projected * inverse_right.unsqueeze(-2)).square().sum(dim=-1)
        / columns
        if gg_left is not None
        else None
    )
...
def kl_soap(update: Tensor, obs, param: Tensor, state: dict[str, Tensor], tempo: Tempo):
    ...
    exp_avg_sq = prior_exp_avg_sq * beta2 + projected.square() * (1 - beta2)
    started = tempo.age > 1
    ...
    preconditioned = _project(exp_avg / exp_avg_sq.sqrt().clamp_min(tempo.hyper.eps), left, right, back=True)
...
def kl_shampoo(update: Tensor, obs, param: Tensor, state: dict[str, Tensor], tempo: Tempo):
    ...
    right = next_state["Q_r"] if "GG_r" in next_state else None
    inverse_left = (
        _factor_inverse(next_state["eigenvalues_l"], tempo.hyper.eps) if "GG_l" in next_state else None
    )
    inverse_right = (
        _factor_inverse(next_state["eigenvalues_r"], tempo.hyper.eps) if "GG_r" in next_state else None
    )
...
def heavy_kl_shampoo(update: Tensor, obs, param: Tensor, state: dict[str, Tensor], tempo: Tempo):
    ...
    right = next_state["Q_r"] if "GG_r" in next_state else None
    inverse_left = (
        _heavy_factor_inverse(next_state["eigenvalues_l"], tempo.hyper.eps) if "GG_l" in next_state else None
    )
    inverse_right = (
        _heavy_factor_inverse(next_state["eigenvalues_r"], tempo.hyper.eps) if "GG_r" in next_state else None
    )
    projected = _project(exp_avg, left, right, back=False)
    preconditioned = projected
    if inverse_left is not None:
